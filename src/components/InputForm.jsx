...
const InputForm = forwardRef(({ type, placeholder, name, label, idElement, error, onChange, onBlur, children }, ref) => {
    // Input
    ...
    return (
        <div className="mb-5">
            <label htmlFor={ idElement } className={ error ? labelErrorClass : labelDefaultClass }>{ label }</label>
            <input
